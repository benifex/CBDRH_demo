...
min(dummy_data1$FAGE)   # 26.7
max(dummy_data1$FAGE)   # 43.6
...
```{r Create: example table 2 using function Part 1}
tab2 <- dummy_data %>% tabyl(PARENT_SEX, N_V_EGTH) %>% tabl_fun() 

```

```{r Create: example table 2 using function Part 2}
tab2 %>% 
...
```{r Create: example table 3 using function Part 1}
tab3 <- dummy_data %>% tabyl(FAGE_BIN, N_V_EGTH) %>% tabl_fun() 

```

```{r Create: example table 3 using function Part 2}
tab3 %>% 
   kable(col.names = c("Age group (years) (a)", "n", "%", "n", "%","n", "%","n", "%"),
         align = "lrrrrrrrr") %>%
   kable_styling(htmltable_class = "lightable-classic-2",
                 full_width = FALSE) %>% 
